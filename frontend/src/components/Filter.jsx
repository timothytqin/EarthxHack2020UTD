...
import { updateType, addFilter, removeFilter } from "../actions/setFilters";
...
const Filter = props => {
  ...
  const filters = (
    <div className={styles.bg}>
      ...
      <FormControl>
        <RadioGroup aria-label="type" value={type} onChange={handleChangeType}>
          <FormControlLabel
            value="laptop"
            control={<GoldRadio />}
            label="laptop"
          ...
          return sum ? (
            <ExpansionPanel key={key} className={classes.root}>
              ...
              <ExpansionPanelDetails className={classes.expansionDetails}>
                <FormControl>
                  <FormGroup>
                    {Object.keys(availablefilters[key]).map(value => {
                      return (
                        <FormControlLabel
                          key={value}
                          control={
                            <GoldCheckbox
                              name={value}
                              checked={reduxFilters[key].includes(value)}
                              onChange={event =>
                                handleCheckBox(
                                  key,
                                  event.target.name,
      ...
      {isSmall ? (
        <div>
          <FilterButton onClick={handleFilterToggle}>
            <FilterListIcon style={{ marginRight: ".5rem" }} /> Filter
          </FilterButton>
          {mobileOpen && (
            <>
              <div>{filters}</div>
